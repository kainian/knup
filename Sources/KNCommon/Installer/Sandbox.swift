...
public final class Sandbox: Sendable {
    ...
    private init() {
        ...
        if let path = ProcessEnv.block[.init("NEXT_PREFIX")] {
            ...
            home = try! .init(validating: "/opt/kn")
        }
        #if DEBUG
        let relativePath = try! RelativePath(validating: "Workspace/kainian/knup")
        ...
        if let path = ProcessEnv.block[.init("NEXT_REPOSITORY")] {
            ...
            bundle = home.appending(component: ".knup")
        }
        #endif
    }
}
...
extension Sandbox {
    ...
    private func findSettingsPath() throws -> AbsolutePath {
        ...
        let relativePath = try! RelativePath(validating: ".knup/Settings.yml")
